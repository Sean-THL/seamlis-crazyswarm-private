...
class ExplorationManager:
    ...
    def _compute_unknown_obstacle_map(self):
        obstacle_map = np.zeros(self.env_handler.get_map_shape(), dtype=np.int8)
        if self.unknown_obs.size == 0:
            return obstacle_map

        robot_padding = 0.0
        if len(self.robot_specs) > 0:
            robot_padding = max(float(spec.get('radius', 0.0)) for spec in self.robot_specs)

        obstacle_geometries = []
        for obs in self.unknown_obs:
            ox, oy, radius = np.asarray(obs[:3], dtype=float)
            if radius <= 0.0:
                continue
            obstacle_geometries.append(Point(float(ox), float(oy)).buffer(float(radius + robot_padding)))

        if len(obstacle_geometries) == 0:
            return obstacle_map

        unknown_geom = unary_union(obstacle_geometries)
        height, width = self.env_handler.get_map_shape()
        y, x = np.meshgrid(np.arange(height), np.arange(width), indexing='ij')
        grid_points = np.column_stack((x.ravel(), y.ravel()))
        cont_points = self.env_handler.grid_to_f(grid_points)
        ...
        contains_mask = np.fromiter(
            (unknown_geom.contains(Point(p)) for p in cont_points),
            dtype=np.bool_,
            count=cont_points.shape[0],
        )
        return contains_mask.reshape(height, width).astype(np.int8)
    ...
    def get_obstacle_map(self):
        obstacle_map = self.static_obstacle_map.copy()
        unknown_obstacle_map = self._compute_unknown_obstacle_map()
        if unknown_obstacle_map.shape == obstacle_map.shape:
            obstacle_map = np.maximum(obstacle_map, unknown_obstacle_map)
        return obstacle_map
